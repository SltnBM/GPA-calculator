...
def calculate_gpa(courses):
    ...
    for name, credits, grade in courses:
        ...
        if weight is None:
            console.print(
                f"[yellow]Invalid grade '{grade}' for course {name}. Skipping.[/]"
            )
            continue
...
def get_category(gpa):
    return (
        ...
        if gpa >= 3.5
            ...
            "Very Good"
            if gpa >= 3.0
            else "Good" if gpa >= 2.5 else "Fair" if gpa >= 2.0 else "Poor"
        )
    )
...
def export_results(course_list, summary, dest_path, fmt):
    ...
    try:
        if fmt in ("json", "all"):
            json_path = (
                dest_path if fmt == "json" else os.path.splitext(dest_path)[0] + ".json"
            )
        ...
        if fmt in ("txt", "all"):
            txt_path = (
                dest_path if fmt == "txt" else os.path.splitext(dest_path)[0] + ".txt"
            )
            with open(txt_path, "w", encoding="utf-8") as f:
                f.write("=== Course Summary ===\n")
                for c in data["courses"]:
                    f.write(
                        f"{c['name']} | Credits: {c['credits']} | Grade: {c['grade']}\n"
                    )
                f.write(
        ...
        if fmt in ("csv", "all"):
            csv_path = (
                dest_path if fmt == "csv" else os.path.splitext(dest_path)[0] + ".csv"
            )
...
def create_json_template(path):
    ...
    try:
        ...
        console.print(
            f"[green]Sample template created at '{path}'. You can edit it and rerun.[/]"
        )
    except IOError as e:
    ...
    if choice == "1":
        while True:
            ...
            if (
                ...
                and safe_input(
                    f"File '{path}' not found. Create sample template here? (y/n): "
                )
                .strip()
                .lower()
                == "y"
    ...
    while True:
        ...
        if multi == "y":
            ...
            for s in range(num_semesters):
                ...
                if not courses:
                    console.print(
                        f"[red]Semester {s+1} has no valid courses. Skipping.[/]"
                    )
                    continue
        ...
        if len(semester_summaries) > 1:
            total_weight = sum(
                s["gpa"] * s["total_credits"] for s in semester_summaries
            )
        ...
        if export == "y":
            ...
            path = (
                safe_input(f"Destination file [{default_name}]: ").strip()
                or default_name
            )
            export_results(
                all_courses,
                {
                    ...
                    "total_credits": sum(
                        s["total_credits"] for s in semester_summaries
                    ),
                },
                path,
                fmt,
        ...
        if (
            safe_input("\nDo you want to input another GPA calculation? (y/n): ")
            .strip()
            .lower()
            != "y"
        ):
